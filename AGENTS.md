...
Adventure Book is a low-code tool that separates content creation from implementation through YAML-based story authoring. Writers can create interactive narratives without touching code, while the application provides a robust, type-safe runtime for story execution.
...
```
YAML Story Files → Parser → Validation → Type-Safe Objects → React Components → User Interface
```

**Key Insight**: The application treats story content as data that flows through a pipeline of transformations, each adding validation and structure.

### Component Hierarchy

- **App**: Route management and error boundaries
- **Introduction/Passage**: Presentational components consuming story data
- **ErrorBoundary**: Centralized error handling for data loading failures
- **Story System**: Pure functions for parsing, validation, and data access

### State Management Philosophy

- **Minimal State**: Use React's built-in state management; avoid external state libraries
- **Persistent State**: localStorage for user progress; everything else is derived from story data
...
The story system transforms YAML content through several stages:
...
- **Content Rules**: Enforce story logic (endings can't have choices, etc.)

## Story Content Separation

### Critical Design Principle: Content Independence

**The most important architectural constraint**: All application code must be completely decoupled from the specific content and structure of any individual story. The tool's primary purpose is to enable authors to test their stories, which means story content will change frequently and unpredictably.

### Separation Requirements

- **No Hard-Coded References**: Never reference specific passage IDs, story titles, or content text in application code
- **Structure Agnostic**: The system must work with any valid story structure (linear, branching, complex graphs)
- **Content Agnostic**: UI components must render any valid content without modification
- **Test Independence**: All tests must work regardless of the actual story content
...
```typescript
// Good: Use mock data that doesn't depend on actual story content
const mockPassage = {
...
// Avoid: Testing with actual story content
const actualPassage = getPassage(1); // Breaks when story changes
```
...
<p>You are in the dark forest...</p>; // Breaks when story changes
```
...
- **Mock External Dependencies**: Always mock `storyLoader` to provide predictable test data

#### Integration Testing Approach

- **User Flow Testing**: Test navigation patterns without depending on specific story paths
- **Error Boundary Testing**: Verify error handling with invalid or missing content
- **State Management Testing**: Ensure persistence works regardless of story structure

### Story Loader Architecture

The story loader acts as the crucial abstraction layer:

```typescript
// Good: Abstract interface that works with any story
...
// The actual story content is an implementation detail
```
...
1. **Story Changes Should Never Break Tests**: If a test fails due to story content changes, the test is wrong
2. **Component Behavior Is Constant**: UI behavior remains consistent regardless of story content
3. **Validation Is Structural**: Validate story structure and format, not specific content values
...
This separation ensures that authors can freely modify their story content, add new passages, change narrative flows, and experiment with different structures without breaking the application or its tests.
...
2. **Validation Errors**: Invalid story content or references
...
1. **Understand Requirements**: What story feature or user need are we addressing?
...
The story system is designed for extension while maintaining content independence:

1. Extend the `Passage` interface in `types.ts`
2. Update validation logic in `storyParser.ts`
3. Modify UI components to handle the new type generically
4. Add tests using mock data, never actual story content

### Integrating External Systems

- **Analytics**: Add event tracking through pure functions that work with any story structure
...
- **Themes**: Expand CSS variables for customization independent of story content

### Key Constraint for Extensions

All extensions must preserve content independence. New features should work with any valid story structure without requiring changes to existing story files or breaking when story content changes.
...
2. **Update Parser**: Add validation logic in `src/data/storyParser.ts`
...
1. **Parser Errors**: Check `StoryParser.validateStory()` output
...
- **Content-Dependent Code**: Never hardcode story content, passage IDs, or specific narrative flows
- **Brittle Tests**: Avoid tests that break when story content changes
